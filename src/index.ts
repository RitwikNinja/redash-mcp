...
// Start the server (stdio for local, HTTP+SSE for remote / Render)
async function main() {
  try {
    const transportMode =
      (process.env.MCP_TRANSPORT ||
        (process.env.PORT ? "sse" : "stdio")).toLowerCase();

    if (transportMode === "stdio") {
      const transport = new StdioServerTransport();

      logger.info("Starting Redash MCP server (stdio)...");
    ...
    // Template server already has all request handlers registered.
    // For each SSE connection we create a fresh MCP server instance and
    // copy over the registered request handlers (excluding init/ping),
    // so multiple simultaneous clients don't interfere with each other.
    const createSessionServer = () => {
      const sessionServer = new Server(
        { name: "redash-mcp", version: "1.1.0" },
        { capabilities: { tools: {}, resources: {} } }
      );

      const templateHandlers = (server as any)._requestHandlers as Map<
        string,
        unknown
      >;
      for (const [method, handler] of templateHandlers.entries()) {
        if (method === "initialize" || method === "ping") continue;
        (sessionServer as any)._requestHandlers.set(method, handler);
      }
    ...
    logger.info(
      `Starting Redash MCP server (SSE) on port ${port} (GET ${ssePath}, POST ${messagePath})...`
    );
